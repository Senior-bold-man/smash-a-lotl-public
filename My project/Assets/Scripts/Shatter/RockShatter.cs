...
{
    public GameObject rockShattered; //this is the variable for the object this will turn into
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.rigidbody == Tool && force >1000)//this checks if (the object colliding is actually the hammer and if its force is larger than what the formula gives us as its breaking point
        {
            Instantiate(rockShattered, transform.position, transform.rotation); //this is just for making sure that the replaced object has the same coordinates (probably don't need to touch)
